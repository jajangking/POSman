...
import { getStoreSettings, updateStoreSettings } from '../services/DatabaseService';
...
const CashierPage: React.FC<CashierPageProps> = ({ onBack, onNavigateToMemberManagement }) => {
      ...
      try {
        ...
        if (settings) {
          setStoreSettings({
            ...
            paperSize: settings.paperSize as '80mm' | '58mm'
          });
        }
      } catch (error) {
  ...
  useEffect(() => {
    const handleKeyPress = (e: KeyboardEvent) => {
      ...
      if (keyboardShortcuts.isShortcut(e, keyboardShortcuts.CASHIER_SHORTCUT)) {
        keyboardShortcuts.preventDefault(e);
        // Focus on the input field for product search
        // In a real implementation, you might want to focus on a specific input
      }
    };
  ...
  // Save store settings
  const saveStoreSettings = async (settings: typeof storeSettings) => {
    try {
      await updateStoreSettings(settings.name, settings.address, settings.phone, settings.paperSize);
