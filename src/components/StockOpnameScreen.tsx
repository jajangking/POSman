...
const StockOpnameScreen: React.FC<StockOpnameScreenProps> = ({ onBack, unfinishedSO, onResumeSO }) => {
  ...
  const handleStartSO = async () => {
    ...
    try {
      const opname = await startStockOpname(soType, new Date().toISOString());
  ...
  const handleAddItem = async (item: InventoryItem) => {
    ...
    try {
      ...
      if (opnameId) {
        await addItemToStockOpname(opnameId, item);
      }
    ...
    <View style={styles.stepContainer}>
      ...
      {soType === 'partial' && (
        <View style={styles.grandSoInfo}>
          <Text style={styles.grandSoText}>Partial SO memungkinkan Anda untuk memilih item tertentu yang akan di SO.</Text>
        </View>
      )}
